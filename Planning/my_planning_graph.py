...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if node_a.prenodes.issubset(self.s_levels[level]):
                # Add this node to the list of action nodes and set parent-child relation with preceding nodes.
                action_nodes.append(node_a)
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)
                    node_a.parents.add(node_s)
        # Create action level from reachable nodes
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            ...
            for node_s in node_a.effnodes:
                # Add this node to the list of state nodes and set parent-child relation with preceding action nodes.
                s_nodes.add(node_s)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        # Make sure node_a1's (+) effects aren't negated by node_a2's (-) effects
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Make sure node_a2's (+) effects aren't negated by node_a1's (-) effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        # node_a1's (+) ADD effects don't conflict with node_a2's (-) preconditions 
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True
                
        # node_a2's (+) ADD effects don't conflict with node_a1's (-) preconditions        
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True

        # node_a1's (-) REM effects don't conflict with node_a2's (+) preconditions        
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True
        
        # node_a2's (-) REM effects don't conflict with node_a1's (+) preconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        # node_a1's preconditions aren't mutually exclusive with node_a2's preconditions
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        '''
        # check if two state literals are not opposite
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # Check node_s1's preconditions aren't mutually exclusive with node_s2's preconditions
