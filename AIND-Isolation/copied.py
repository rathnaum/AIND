...
def custom_score(game, player):
    ...
    """
    return custom_score_weighted(game, player, 1.5, 1)


def custom_score_weighted(game, player, my_weight=1, opp_weight=1.5):
...
class CustomPlayer:
    ...
    def max_value(self, game, depth, alpha=None, beta=None):
        """ Function to return max value from the tree
        Parameters
        ----------
        game : isolation.Board
            An instance of the Isolation game `Board` class representing the current game state
        depth : int
            Depth is an integer representing the maximum number of levels to search in the game tree before aborting
        alpha : float
            Alpha limits the lower bound of search on minimizing layers
        beta : float
            Beta limits the upper bound of search on maximizing layers
        ...
        -------
        float
            The score for the max for current search branch
        """
        ...
        for move in available_my_moves:
            # returns the max value of all branches, as the intent is to maximize opponents score
    ...
    def min_value(self, game, depth, alpha=None, beta=None):
            """ Function to return min value from the tree branch
        Parameters
        ----------
        game : isolation.Board
            An instance of the Isolation game `Board` class representing the current game state
        ...
        -------
        float
            The score for the min for current search branch
        """
        if self.should_timeout():
            raise Timeout()
        ...
        for move in available_opponent_moves:
            # returns the min value of all branches, as the intent is to minimize opponents score
